...
@router.get("")
def read_settings(request: Request, session: Session = Depends(db_session)):
    """班级信息 + 存储占用 + 每张业务表的行数。"""
    return {"ok": True, "data": settings_view(session, _class_id(request, session))}


@router.put("/class")
def put_class(request: Request, body: dict = Body(default_factory=dict), session: Session = Depends(db_session)):
...
@router.post("/clear")
def post_clear(request: Request, body: dict = Body(default_factory=dict), session: Session = Depends(db_session)):
...
@router.get("/class")
def read_class(request: Request, session: Session = Depends(db_session)):
