...
@router.get("/board")
def get_board(request: Request, session: Session = Depends(db_session)):
...
@router.put("/plan")
def put_plan(
    request: Request, body: dict = Body(default_factory=dict), session: Session = Depends(db_session)
):
...
@router.get("/plan")
def read_plan(request: Request, session: Session = Depends(db_session)):
    class_id = resolve_class_id(SEAT, request.query_params.get("classId"), session)
